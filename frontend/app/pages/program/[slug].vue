<template>
    <div class="bg-slate-50 dark:bg-emerald-950 text-slate-900 dark:text-white min-h-screen pb-20">
        <!-- Hero Section with Background image and mosque shadow overlay -->
        <section
        ...
        >
            ...
            <div class="relative max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
                ...
                >
                    ...
                    />
                    <span>{{ locale === 'en' ? 'Back to Programs' : 'Kembali ke Program' }}</span>
                </NuxtLink>
                ...
                <div v-else-if="program" class="max-w-4xl" data-aos="fade-up">
                    ...
                    <div class="mt-6 flex flex-wrap gap-4 text-sm text-slate-600 dark:text-emerald-100/70">
                        <span class="inline-flex items-center gap-1.5">
                            <UIcon name="i-lucide-award" class="w-4.5 h-4.5 text-amber-500" />
                            Sertifikasi IAO
                        </span>
                        <span class="inline-flex items-center gap-1.5">
                            <UIcon name="i-lucide-trees" class="w-4.5 h-4.5 text-emerald-600 dark:text-emerald-400" />
                            Tadabbur Alam
                        </span>
                        <span class="inline-flex items-center gap-1.5">
                            <UIcon name="i-lucide-shield-check" class="w-4.5 h-4.5 text-emerald-600 dark:text-emerald-400" />
                            Legalitas Resmi
                        </span>
                    </div>
                </div>
        ...
        <section class="py-12 lg:py-16">
            <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
                ...
                <div v-else-if="pending" class="grid grid-cols-1 lg:grid-cols-12 gap-8">
                ...
                <!-- Program Detail Grid -->
                <div v-else-if="program" class="grid grid-cols-1 lg:grid-cols-12 gap-10 items-start">
                    <!-- Left: Program Image & Description -->
                    <div class="lg:col-span-7 xl:col-span-8 space-y-8" data-aos="fade-right">
                        <!-- Image Gallery/Showcase Card -->
                        <div
                        ...
                        >
                            ...
                            >
                                {{ locale === 'en' ? 'About this Program' : 'Tentang Program Ini' }}
                            </h2>
                            <div
                    ...
                    <!-- Right: Sticky Purchase/Action Card -->
                    <div class="lg:col-span-5 xl:col-span-4 lg:sticky lg:top-24" data-aos="fade-left">
                        <div
                        ...
                        >
                            <!-- Price Tag -->
                            <div>
                                <span class="text-xs font-semibold text-slate-500 dark:text-emerald-200/60 uppercase tracking-widest block mb-1">
                                    {{ locale === 'en' ? 'Investment Price' : 'Biaya Investasi' }}
                                </span>
                                <div class="flex items-baseline gap-2">
                                    <span class="text-3xl md:text-4xl font-serif font-extrabold text-emerald-800 dark:text-amber-400">
                            ...
                            <div class="space-y-3.5">
                                <h4 class="text-sm font-bold text-slate-900 dark:text-white">
                                    {{ locale === 'en' ? 'Program Features' : 'Fitur Program' }}
                                </h4>
                                <ul class="space-y-2.5 text-sm text-slate-600 dark:text-emerald-100/80">
                                    <li class="flex items-start gap-2.5">
                                        <UIcon name="i-lucide-check-circle-2" class="w-5 h-5 text-emerald-600 dark:text-emerald-400 shrink-0 mt-0.5" />
                                        <span>{{ locale === 'en' ? 'Accredited by IAO (International)' : 'Akreditasi IAO Internasional' }}</span>
                                    </li>
                                    <li class="flex items-start gap-2.5">
                                        <UIcon name="i-lucide-check-circle-2" class="w-5 h-5 text-emerald-600 dark:text-emerald-400 shrink-0 mt-0.5" />
                                        <span>{{ locale === 'en' ? 'Stress-free Exam Concept' : 'Metode Ujian Bebas Stres' }}</span>
                                    </li>
                                    <li class="flex items-start gap-2.5">
                                        <UIcon name="i-lucide-check-circle-2" class="w-5 h-5 text-emerald-600 dark:text-emerald-400 shrink-0 mt-0.5" />
                                        <span>{{ locale === 'en' ? 'Outdoor Nature Activities' : 'Tadabbur Alam Terintegrasi' }}</span>
                                    </li>
                                    <li class="flex items-start gap-2.5">
                                        <UIcon name="i-lucide-check-circle-2" class="w-5 h-5 text-emerald-600 dark:text-emerald-400 shrink-0 mt-0.5" />
                                        <span>{{ locale === 'en' ? 'Official Certification' : 'Sertifikat Kelulusan Resmi' }}</span>
                                    </li>
                                </ul>
                            </div>
                            ...
                            <div class="space-y-3">
                                ...
                                >
                                    <UIcon :name="copied ? 'i-lucide-check' : 'i-lucide-share-2'" class="w-4.5 h-4.5" />
                                    <span>{{ copied ? (locale === 'en' ? 'Copied!' : 'Berhasil Disalin!') : (locale === 'en' ? 'Share Program' : 'Bagikan Program') }}</span>
                                </button>
                            </div>
                        </div>
                    </div>
...
const titleOf = (p: any) =>
    locale.value === "en" ? p.title_en : p.title_id;
const descOf = (p: any) =>
    locale.value === "en" ? p.description_en : p.description_id;
const priceOf = (p: any) =>
    locale.value === "en" ? p.price_en : p.price_id;
...
useSeoMeta({
    title: () => (program.value ? `${titleOf(program.value)} - YHIE` : "Program Detail - YHIE"),
    description: () => (program.value ? descOf(program.value).substring(0, 160) : "Detail program Yayasan Hafiz Indonesia Emas."),
    ogTitle: () => (program.value ? `${titleOf(program.value)} - YHIE` : "Program Detail - YHIE"),
    ogDescription: () => (program.value ? descOf(program.value).substring(0, 160) : "Detail program Yayasan Hafiz Indonesia Emas."),
});
</script>
