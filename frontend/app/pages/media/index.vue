<template>
    <div class="bg-slate-50 dark:bg-emerald-950 text-slate-900 dark:text-white min-h-screen">
        ...
        <!-- Filters Section -->
        <section class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 mt-12 flex flex-wrap justify-center gap-3">
            <button
        ...
        <section class="py-10 lg:py-14">
            <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
                <!-- Loading Skeleton -->
                <div v-if="pending" class="columns-2 md:columns-3 gap-5 space-y-5">
                    <div
                        v-for="n in 9"
                        :key="n"
                        class="break-inside-avoid rounded-2xl overflow-hidden"
                        :class="n % 3 === 0 ? 'h-72' : n % 3 === 1 ? 'h-56' : 'h-44'"
                    >
                ...
                <div v-else-if="!mediaItems.length" class="text-center py-20">
                    <div class="mx-auto w-20 h-20 rounded-3xl bg-emerald-100 dark:bg-emerald-900/60 flex items-center justify-center">
                        <UIcon name="i-lucide-image-off" class="w-10 h-10 text-emerald-500 dark:text-emerald-400" />
                    </div>
                    <p class="mt-5 text-slate-500 dark:text-emerald-100/70 text-lg">
                        {{ locale === 'en' ? 'No gallery items yet.' : 'Belum ada item galeri.' }}
                    </p>
                </div>
                    ...
                    >
                        ...
                        >
                            ...
                            >
                                ...
                                />
                                <div v-else class="w-full h-full flex items-center justify-center bg-gradient-to-br from-emerald-100 to-amber-50 dark:from-emerald-900/40 dark:to-emerald-950/60">
                                ...
                                >
                                    <div>
                                        <span class="inline-block text-xs font-bold uppercase tracking-[0.2em] text-amber-400 mb-2">
                                            {{ getCategoryLabel(item.category) }}
                                        </span>
                                        <h3 class="text-white font-serif font-bold text-lg leading-snug">
                                            {{ titleOf(item) }}
                                        </h3>
                                        <p class="mt-1.5 text-emerald-100/80 text-sm line-clamp-2 leading-relaxed">
                                            {{ descOf(item) }}
                                        </p>
                                    </div>
                                </div>
                            ...
                            <div class="p-4 group-hover:bg-emerald-50/50 dark:group-hover:bg-emerald-900/30 transition-colors duration-300">
                                <h3 class="font-serif font-bold text-sm text-emerald-950 dark:text-emerald-50 line-clamp-1">
                                    {{ titleOf(item) }}
                                </h3>
                                <p class="mt-1 text-xs text-slate-500 dark:text-emerald-100/50 line-clamp-1">
                                    {{ descOf(item) }}
                                </p>
                            </div>
                        </div>
                ...
                >
                    ...
                    <div class="max-w-5xl mx-auto px-4 text-center" @click.stop>
                        ...
                        <div v-if="currentLightboxItem" class="mt-5">
                            ...
                            <p class="mt-2 text-white/60 text-sm max-w-2xl mx-auto">
...
const getCardHeight = (index: number) => {
    ...
    switch (pattern) {
        case 0: return "h-64 sm:h-72";
        case 1: return "h-48 sm:h-56";
        case 2: return "h-56 sm:h-64";
        case 3: return "h-44 sm:h-52";
        case 4: return "h-60 sm:h-72";
        case 5: return "h-48 sm:h-56";
        default: return "h-56";
    }
};
...
onMounted(() => {
    const handler = (e: KeyboardEvent) => {
        ...
        if (e.key === "ArrowRight" && lightboxIndex.value < mediaItems.value.length - 1) lightboxIndex.value++;
    };
