<template>
    ...
    >
        ...
        <div v-if="submitSuccess" class="p-8 md:p-12 text-center">
            <div class="mx-auto w-20 h-20 rounded-full bg-gradient-to-br from-emerald-400 to-emerald-600 flex items-center justify-center shadow-xl shadow-emerald-500/30 mb-6 animate-bounce-once">
                <UIcon name="i-lucide-check" class="w-10 h-10 text-white" />
            </div>
            <h3 class="text-2xl font-serif font-extrabold text-emerald-900 dark:text-emerald-50 mb-2">
        ...
        <div v-else class="flex flex-col max-h-[85vh]">
            <!-- Modal Header -->
            <div class="relative overflow-hidden px-6 pt-6 pb-5 border-b border-emerald-200/40 dark:border-emerald-800/40 shrink-0">
                <!-- Decorative blurs -->
                <div class="absolute -top-10 -right-10 w-32 h-32 bg-amber-400/15 rounded-full blur-2xl"></div>
                <div class="absolute -bottom-8 -left-8 w-28 h-28 bg-emerald-400/15 rounded-full blur-2xl"></div>

                <div class="relative flex items-start justify-between">
                    <div>
                        <span class="inline-flex items-center gap-1.5 text-[10px] font-bold uppercase tracking-[0.25em] text-amber-600 dark:text-amber-400 mb-2">
                            <UIcon name="i-lucide-pen-line" class="w-3.5 h-3.5" />
                            {{ t('registration.selectedProgram') }}
                        </span>
                        <h2 class="text-xl md:text-2xl font-serif font-extrabold text-emerald-950 dark:text-emerald-50 leading-tight">
                ...
                <!-- Program Info Badge -->
                <div class="mt-4 flex items-center gap-3 p-3 rounded-2xl bg-emerald-50/80 dark:bg-emerald-900/30 border border-emerald-200/50 dark:border-emerald-800/50">
                    <div class="w-10 h-10 rounded-xl bg-gradient-to-br from-emerald-500 to-emerald-600 flex items-center justify-center shadow-md shrink-0">
                        <UIcon name="i-lucide-graduation-cap" class="w-5 h-5 text-white" />
                    </div>
                    <div class="min-w-0 flex-1">
                        <p class="text-sm font-bold text-emerald-900 dark:text-emerald-50 truncate">
                            {{ programTitle }}
                        </p>
            ...
            <!-- Form Body -->
            <form @submit.prevent="handleSubmit" class="px-6 py-5 space-y-6 flex-1 min-h-0 overflow-y-auto custom-scrollbar">
                ...
                <div>
                    ...
                    <div class="space-y-4">
                        <!-- Full Name -->
                        <div>
                            <label class="block text-xs font-semibold text-slate-600 dark:text-emerald-200/70 mb-1.5">
                                {{ t('registration.fullName') }} <span class="text-red-500">*</span>
                            </label>
                            <input
                            ...
                            />
                            <p v-if="errors.full_name" class="mt-1 text-xs text-red-500">{{ errors.full_name[0] }}</p>
                        </div>
                        ...
                        <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                            <div>
                                <label class="block text-xs font-semibold text-slate-600 dark:text-emerald-200/70 mb-1.5">
                                    {{ t('registration.email') }} <span class="text-red-500">*</span>
                                </label>
                                <input
                                    v-model="form.email"
                                    type="email"
                            ...
                            <div>
                                <label class="block text-xs font-semibold text-slate-600 dark:text-emerald-200/70 mb-1.5">
                                    {{ t('registration.phone') }} <span class="text-red-500">*</span>
                                </label>
                                <input
                                    v-model="form.phone"
                                    type="tel"
                        ...
                        <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                            <div>
                                <label class="block text-xs font-semibold text-slate-600 dark:text-emerald-200/70 mb-1.5">
                                    {{ t('registration.gender') }} <span class="text-red-500">*</span>
                                </label>
                                <div class="flex gap-3">
                                    ...
                                    >
                                        ...
                                        <span class="font-medium">{{ t('registration.male') }}</span>
                                    </label>
                                    <label
                                        class="flex-1 flex items-center gap-2.5 px-4 py-3 rounded-xl border cursor-pointer transition-all duration-300 text-sm"
                                    ...
                                    >
                                        ...
                                        <UIcon name="i-lucide-user" class="w-4 h-4" />
                                        <span class="font-medium">{{ t('registration.female') }}</span>
                                    </label>
                                </div>
                                <p v-if="errors.gender" class="mt-1 text-xs text-red-500">{{ errors.gender[0] }}</p>
                            </div>
                            <div>
                                <label class="block text-xs font-semibold text-slate-600 dark:text-emerald-200/70 mb-1.5">
                                    {{ t('registration.age') }}
                                ...
                                <input
                                    ...
                                    class="w-full px-4 py-3 rounded-xl border border-emerald-200/70 dark:border-emerald-800/60 bg-white/60 dark:bg-emerald-900/20 text-slate-900 dark:text-white placeholder-slate-400 dark:placeholder-emerald-300/40 focus:outline-none focus:ring-2 focus:ring-emerald-500/40 focus:border-emerald-400 dark:focus:border-emerald-500 transition-all duration-300 text-sm"
                                />
                                <p v-if="errors.age" class="mt-1 text-xs text-red-500">{{ errors.age[0] }}</p>
                            </div>
                        </div>
                    </div>
                </div>
                ...
                <!-- Divider -->
                <hr class="border-emerald-200/40 dark:border-emerald-800/40" />

                <!-- Additional Info Section -->
                <div>
                    <div class="flex items-center gap-2 mb-4">
                        <div class="w-7 h-7 rounded-lg bg-amber-100 dark:bg-amber-900/30 flex items-center justify-center">
                            <UIcon name="i-lucide-map-pin" class="w-4 h-4 text-amber-600 dark:text-amber-400" />
                        </div>
                        <h3 class="text-sm font-bold text-slate-900 dark:text-white uppercase tracking-wider">
                ...
                <!-- General Error -->
                <div v-if="generalError" class="p-3 rounded-xl bg-red-50 dark:bg-red-950/30 border border-red-200 dark:border-red-800/50 text-sm text-red-700 dark:text-red-300 flex items-start gap-2">
            ...
            <!-- Modal Footer -->
            <div class="px-6 py-4 border-t border-emerald-200/40 dark:border-emerald-800/40 flex flex-col sm:flex-row gap-3 sm:justify-end shrink-0">
                <button
                    @click="closeAndReset"
                    type="button"
                ...
                >
                    ...
                    <UIcon
                        ...
                        class="w-4 h-4"
                    />
                    <span>{{ isSubmitting ? t('registration.submitting') : t('registration.submit') }}</span>
                </button>
            </div>
        </div>
        </template>
